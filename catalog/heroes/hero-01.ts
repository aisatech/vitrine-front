...
<section class="relative w-full overflow-hidden bg-[var(--color-bg)]" style="background-image:url('${ASSET_PATHS.heroBg}'); background-size:cover; background-position:center;">
  <div class="mx-auto flex max-w-5xl flex-col items-center gap-6 px-6 py-24 text-center">
    <img src="${ASSET_PATHS.profile}" alt="Foto de perfil" class="h-32 w-32 rounded-full border-4 border-[var(--color-primary)] object-cover" />
    <h1 class="text-4xl font-bold text-[var(--color-text-main)] sm:text-5xl">Olá, eu sou <span class="text-[var(--color-primary)]">Seu Nome</span></h1>
    <p class="max-w-xl text-lg text-[var(--color-text-muted)]">Desenvolvedor(a) focado em criar experiências digitais rápidas e acessíveis.</p>
